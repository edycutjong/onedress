...
/**
 * The product thesis, asserted where a judge actually meets it: `/party`, the app.
 * (The marketing landing page at `/` is covered by marketing.spec.ts.) These numbers
 * come from the real engine running on the demo party at module load — if the engine
 * or the fixture drifts, these fail, which is exactly what they are for.
 */

const spine = (page: import('@playwright/test').Page) =>
  page.getByRole('navigation', { name: 'Progress' });

test.describe('verdict — the app’s opening screen', () => {
